...
This final subsection will apply the Higgs mechanism to the electroweak sector of the Standard Model, with a \Ugroup{1}$_Y\times$\SUgroup{2}$_{L}$ local gauge symmetry. 
\begin{equation}
    \phi=
    \begin{pmatrix}
         \phi^+ \\
...
\end{equation}
It is known, however, that the photon is indeed massless and must remain so after symmetry breaking. It is for this reason that only the neutral scalar field $phi^0$ can acquire a non-zero vacuum expectation value \todo{This is REALLY badly worded and makes no sense. Read more into it and reword}.
\begin{equation}
