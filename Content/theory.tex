...
In order to maintain local gauge invariance under \SUgroup{3} gauge transformations of the form
\begin{equation}
...
\end{equation}
where $\lambda_a$ with $a=1,2,...,8$ are the $3\times3$ Gell-Mann matrices, whose role for \SUgroup{3} is analogous to the role Pauli spin matrices play for \SUgroup{2}. For the new gauge fields $G^a$, it will suffice to know the approximate transformation rule. For that the infinitesimal case is considered, where only first-order terms are kept. The transformation rule obtained is
\begin{equation}
...
Two important features of QCD are asymptotic freedom and confinement. Asymptotic freedom, describes the behaviour of quarks at high energies or, equivalently, at small distances. In this regime, quarks
...
\textcolor{red}{REPHRASE THIS WHOLE THING!!!}
