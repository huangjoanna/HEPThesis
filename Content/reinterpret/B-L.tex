...
\end{equation}
Here the parameter $\mu$ dictates the strength of the BSM signal process. $\mu = 0$ and $\mu = 1$ correspond to the background-only hypothesis and the nominal signal hypothesis respectively \cite{m4l2021_paper}. The reason for setting $\tilde{q} = 0$ for $\hat{\mu} > \mu$ is that when setting an upper limit, an excess in data (corresponding to larger $\hat{\mu}$) should not be used to reject a signal model with lower yield (lower $\mu$). $\tilde{q}$ is interpreted as an upper
...
\begin{figure}
    ...
    \caption{ATLAS result from the \mFourL{} measurement. Limits set on the B-L model using only the inclusive \mFourL{} variable. The red line shows the observed exclusion and the solid black line shows the expected exclusion, with the dashed black lines indicating the $\pm 1\sigma$ and $\pm 2\sigma$ uncertainty bands. The region above and to the left of the lines is excluded, up to the grey band, which shows the region $m_{h_2} < 2m_{Z'}$, where the four-lepton final state has no sensitivity. This figure is from Reference~\cite{m4l2021_paper}.}
...
\begin{figure}
    ...
    \caption{ATLAS result from the \mFourL{} measurement. Limits set on the B-L model using the most sensitive variable. The red line shows the observed exclusion and the solid black line shows the expected exclusion, with the dashed black lines indicating the $\pm 1\sigma$ and $\pm 2\sigma$ uncertainty bands. The region above and to the left of the lines is excluded, up to the grey band, which shows the region $m_{h_2} < 2m_{Z'}$, where the four-lepton final state has no sensitivity. This figure is from Reference~\cite{m4l2021_paper}.}
    \label{fig:BLcontour}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=\mediumfigwidth]{Figures/m4l/Interpretations/UpperLimitBandWithContour_2D_colormap.pdf}
    \caption{ATLAS result from the \mFourL{} measurement. This colour map shows the B-L model space and the most sensitive variable at each point used to set the limit. This figure is from Reference~\cite{m4l2021_paper}.}
...
\begin{figure}[tbp]
    ...
    \end{subfigure}
    \caption{\contur exclusion (a) without and (b) with the new four-lepton measurement. The disfavoured regions at 95\% CL and 68\% CL are shaded in yellow and green respectively.}
    \label{fig:BL:4l}
\end{figure}
\begin{figure}[tbp]
    \centering
    ...
    \end{tabular}
    \caption{The most sensitive distribution from the \ATLAS four-lepton measurement contributing to the limit is indicated in colour for the gauged $B-L$ model in $M_{h_2}$ vs $\sin\alpha$ plane, where $g'_1=0.001$ and $M_{Z'} = 35$~\GeV.}
    \label{fig:BL:m4ldom}
\end{figure}
